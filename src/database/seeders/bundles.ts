...
const bundleRows: BundleSeedRow[] = [
  ...
  {
    code: "DS-BI-CW-0001",
    name: "Production Crew (UPM, PU, Runner)",
    description: "Paket hemat untuk kebutuhan produksi dasar.",
    base_price: 0,
    discount_type: "percent",
    discount_value: 10,
    final_price: 0,
    code_items: [
      { code: "DS-CW-UPM-0001", quantity: 10 },
      { code: "DS-CW-PU-0001", quantity: 10 },
      { code: "DS-CW-RN-0001", quantity: 10 },
    ],
  },
];
...
function getBundleTypeCode(code: string) {
  const parts = code.split("-");
  return parts[1] ?? "";
}

function getBundleCategoryCode(code: string) {
  const parts = code.split("-");
  return parts[2] ?? "";
}
...
export async function seedBundles(
  ...
  const bundleTypeCodes = Array.from(
    new Set(
      bundleRows.map((row) => getBundleTypeCode(row.code)).filter(Boolean),
    ),
  );
  const bundleCategoryCodes = Array.from(
    new Set(
      bundleRows.map((row) => getBundleCategoryCode(row.code)).filter(Boolean),
    ),
  );

  const [bundleTypeResult, bundleCategoryResult] = await Promise.all([
    bundleTypeCodes.length
      ? supabase
          .from("bundle_types")
          .select("id, short_code")
          .in("short_code", bundleTypeCodes)
      : Promise.resolve({ data: [], error: null }),
    bundleCategoryCodes.length
      ? supabase
          .from("bundle_categories")
          .select("id, short_code")
          .in("short_code", bundleCategoryCodes)
      : Promise.resolve({ data: [], error: null }),
  ]);

  if (bundleTypeResult.error) {
    throw new Error(
      `Select bundle_types failed: ${bundleTypeResult.error.message}`,
    );
  }
  if (bundleCategoryResult.error) {
    throw new Error(
      `Select bundle_categories failed: ${bundleCategoryResult.error.message}`,
    );
  }

  const bundleTypeMap = new Map(
    (bundleTypeResult.data ?? []).map((row) => [
      String(row.short_code ?? ""),
      Number(row.id),
    ]),
  );
  const bundleCategoryMap = new Map(
    (bundleCategoryResult.data ?? []).map((row) => [
      String(row.short_code ?? ""),
      Number(row.id),
    ]),
  );
  ...
  const bundleTypeRows: BundleRelationRow[] = [];
  const bundleCategoryRows: BundleRelationRow[] = [];

  bundleRows.forEach((bundle) => {
    const bundleId = insertedByCode.get(bundle.code);
    if (!bundleId) return;

    const typeCode = getBundleTypeCode(bundle.code);
    const typeId = bundleTypeMap.get(typeCode);
    if (typeId) {
      bundleTypeRows.push({ bundle_id: bundleId, related_id: typeId });
    }

    const categoryCode = getBundleCategoryCode(bundle.code);
    const categoryId = bundleCategoryMap.get(categoryCode);
    if (categoryId) {
      bundleCategoryRows.push({ bundle_id: bundleId, related_id: categoryId });
    }
  });

  if (bundleTypeRows.length > 0) {
    const bundleIds = Array.from(
      new Set(bundleTypeRows.map((row) => row.bundle_id)),
    );
    ...
    if (error) {
      throw new Error(`Select bundle_type failed: ${error.message}`);
    }
    ...
    const rowsToInsert = bundleTypeRows.filter(
      (row) => !existingKey.has(`${row.bundle_id}-${row.related_id}`),
    );

    if (rowsToInsert.length > 0) {
      const { error: insertError } = await supabase.from("bundle_type").insert(
        rowsToInsert.map((row) => ({
          bundle_id: row.bundle_id,
          bundle_type_id: row.related_id,
        })),
      );
  ...
  if (bundleCategoryRows.length > 0) {
    const bundleIds = Array.from(
      new Set(bundleCategoryRows.map((row) => row.bundle_id)),
    );
    const { data: existingPairs, error } = await supabase
    ...
    if (error) {
      throw new Error(`Select bundle_category failed: ${error.message}`);
    }
    ...
    const rowsToInsert = bundleCategoryRows.filter(
      (row) => !existingKey.has(`${row.bundle_id}-${row.related_id}`),
    );
    ...
    if (rowsToInsert.length > 0) {
      const { error: insertError } = await supabase
        ...
        .insert(
          rowsToInsert.map((row) => ({
            bundle_id: row.bundle_id,
            bundle_category_id: row.related_id,
          })),
        );

      if (insertError) {
        throw new Error(
          `Insert bundle_category failed: ${insertError.message}`,
        );
      }
    }
  }
