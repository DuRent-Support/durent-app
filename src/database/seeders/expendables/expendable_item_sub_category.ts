...
const parseSubCategory = (code: string) => {
  const parts = code.split("-");
  return parts.length >= 4 ? parts[3] : null;
};

export async function seedExpendableItemSubCategory(
  ...
  if (!items || items.length === 0) {
    return {
      table: "expendable_item_sub_category",
      total: 0,
      inserted: 0,
      skipped: 0,
    };
  ...
  const subCategoryCodes = Array.from(
    new Set(
      items
        .map((row) => parseSubCategory(String(row.code ?? "")))
  ...
  for (const item of items as ExpendableRow[]) {
    const subCategoryCode = parseSubCategory(String(item.code ?? ""));
    if (!subCategoryCode) {
  ...
  if (existingError) {
    throw new Error(
      `Select expendable_item_sub_category failed: ${existingError.message}`,
    );
  }

  const existingKey = new Set(
    (existingPairs ?? []).map(
      (row) => `${row.expendable_id}-${row.item_sub_category_id}`,
    ),
  );

  const rowsToInsert = desiredPairs.filter(
    (row) =>
      !existingKey.has(`${row.expendable_id}-${row.item_sub_category_id}`),
  );
  ...
  if (rowsToInsert.length > 0) {
    ...
    if (insertError) {
      throw new Error(
        `Insert expendable_item_sub_category failed: ${insertError.message}`,
      );
    }
  }
