...
export function MasterDataCrudPage({
  ...
  endpoint,
  entityLabel,
}: MasterDataCrudPageProps) {
  const [records, setRecords] = useState<MasterDataItem[]>([]);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [saveConfirmOpen, setSaveConfirmOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<MasterDataItem | null>(
    null,
  );
  const [deleteConfirm, setDeleteConfirm] = useState<MasterDataItem | null>(
    null,
  );
  ...
  const validateForm = () => {
    ...
    if (!formData.short_code.trim()) {
      toast.error("Short code wajib diisi.");
      return false;
    }
    ...
    try {
      ...
      if (!response.ok) {
        toast.error(
          result.message ||
            (editingRecord
              ? `Gagal mengupdate ${entityLabel}.`
              : `Gagal menambahkan ${entityLabel}.`),
        );
        return;
      }
      ...
      toast.success(
        editingRecord
          ? `${entityLabel} berhasil diupdate.`
          : `${entityLabel} berhasil ditambahkan.`,
      );
  ...
  const deleteRecord = async (item: MasterDataItem) => {
    try {
      ...
      if (!response.ok) {
        toast.error(result.message || `Gagal menghapus ${entityLabel}.`);
        return;
      }
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  ...
                  >
                    Belum ada data. Klik &quot;Tambah {entityLabel}&quot; untuk
                    menambahkan.
                  </TableCell>
                </TableRow>
              ) : (
        ...
        <DialogContent className="border-border sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="font-display">
              {editingRecord ? `Edit ${entityLabel}` : `Tambah ${entityLabel}`}
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="grid gap-1.5">
              ...
              <Input
                ...
                onChange={(event) =>
                  setFormData((prev) => ({
                    ...prev,
                    short_code: event.target.value,
                  }))
                }
                placeholder="Masukkan short code"
              />
            </div>
          </div>
          <DialogFooter>
